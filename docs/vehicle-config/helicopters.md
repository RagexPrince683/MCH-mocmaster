...
| `UseNewHelicopterFlightModel` / `EnableNewHelicopterFlightModel` | boolean | false | Helicopter-specific opt-in gate for the new helicopter flight model. This is intentionally separate from `UseNewMobilitySystem`; when false, all new helicopter physical tuning values are inert and legacy helicopter flight remains unchanged. |
| `PhysicalMass` | float >= 0.01 | 1.0 | Relative mass for rotor thrust, gravity, vertical drag, and inertia calculations. `1.0` is the legacy-scale baseline rather than real kilograms. |
...
| `RotorInertia` | float >= 0.01 | 1.0 | Rotor RPM acceleration resistance; larger values make RPM change more slowly. |
| `RotorSpoolUpRate` | 0.0-1.0 | 0.02 | Normalized rotor RPM gain per tick while spooling up. |
| `RotorSpoolDownRate` | 0.0-1.0 | 0.03 | Normalized rotor RPM loss per tick while spooling down. |
| `CollectiveResponse` | 0.0-1.0 | 0.08 | Smoothing rate for collective/lift input changes. |
| `CyclicAuthority` | float >= 0 | 1.0 | Pitch/roll cyclic authority multiplier. |
| `TailRotorAuthority` | float >= 0 | 1.0 | Yaw/tail-rotor authority multiplier. |
| `YawDamping` | float >= 0 | 0.15 | Yaw stabilization/damping multiplier. |
| `AngularInertia` | float >= 0.01 | 1.0 | Relative rotational inertia for angular acceleration. |
| `TranslationalLiftCoefficient` | float >= 0 | 0.004 | Forward-speed lift bonus coefficient; default mirrors the current translational-lift cap scale. |
| `VerticalDrag` | float >= 0 | 0.02 | Vertical climb/descent damping coefficient. |
| `MaxClimbRate` / `HelicopterMaxClimbRate` / `NewHelicopterMaxClimbRate` | blocks/tick >= 0 | 0.16 | New-model maximum sustained upward velocity after lift and vertical drag. Parser clamps configured values to `0.22` blocks/tick so normal climbs stay believable; lighter/older helicopters should usually sit around `0.13`-`0.14`, modern attack/utility helicopters around `0.15`-`0.16`, and heavy transports around `0.12`-`0.13`. `0` disables the cap. |
| `ParasiteDrag` | float >= 0 | 0.01 | Horizontal air-drag coefficient. Increase this to damp horizontal speed and make larger helicopters feel heavier. |
| `HorizontalRotorThrustScale` / `HelicopterHorizontalThrustScale` | float >= 0 | 0.35 | New-model-only Minecraft-scale multiplier for converting pitch-attitude rotor thrust into forward/backward acceleration. Lower values preserve hover/climb tuning while reducing map-scale forward acceleration; tuned helicopter assets commonly use about `1.10`-`1.72` to set role-based forward top-speed feel without changing lift. |
...
| `HoverAssistStrength` | 0.0-1.0 | 0.75 | New-model hover assistance strength. `0.0` disables assist; the default now provides strong altitude and horizontal-drift stabilization when hover/gunner hover assist is active and the pilot is not commanding collective/cyclic input. |
| `HoverPitchStabilizerStrength` / `HoverAttitudeStabilizerStrength` | float >= 0 | 0.045 | New-model hover/gunner attitude-leveling strength. Raises cyclic correction toward level pitch/roll without snapping vehicle orientation or changing pilot collective. |
...
The current controlled asset opt-in set is intentionally small and conservative. `ah-6`, `uh-60ja`, and `ach47a` provide light, medium utility, and heavy armed-transport test coverage without globally changing the helicopter folder. Their values are current flyability baselines, not final realism or balance targets.

Early in-game tuning should check the effective thrust-to-weight relationship before increasing cyclic, yaw, or hover assistance. At full spool (`normalizedRotorRPM` near `1.0`) and full collective, `MainRotorMaxThrust` is the primary hover/climb power knob: compare `MainRotorMaxThrust * rotorEfficiency` against `PhysicalMass * gravity`, then account for `VerticalDrag`, `ParasiteDrag`, and modest translational-lift/hover-assist modifiers. If an opted-in helicopter cannot lift off, prefer small `MainRotorMaxThrust` increases first, use `PhysicalMass` reductions sparingly, and do not use cyclic authority or extreme `TranslationalLiftCoefficient` as takeoff crutches. After lift is correct, tune pitch-driven forward feel primarily with `HorizontalRotorThrustScale`, then use `ParasiteDrag` as forward speed damping and `CyclicAuthority` for role-based responsiveness; heavier helicopters should generally use lower cyclic/scale and higher drag than light helicopters. Current bundled assets treat the Hind/medium attack class as a `1.60` forward-speed baseline, with lighter scout/attack helicopters only modestly above it and heavy transports below it. Sideways movement comes from roll attitude and is intentionally less efficient: tune `HelicopterLateralThrustScale`, `HelicopterLateralDrag`, and `HelicopterMaxLateralSpeedScale` instead of reducing roll authority. Backward movement can be made weaker than forward flight with `HelicopterBackwardThrustScale` and `HelicopterMaxBackwardSpeedScale`; when those optional values are omitted, backward thrust and speed limits inherit the forward behavior for compatibility. Avoid stacking very low thrust scale with very high drag, because that makes cyclic input feel stuck instead of merely controlled.


### Rotor RPM foundation

When `UseNewHelicopterFlightModel = true`, helicopters now maintain a normalized runtime rotor state for lift and yaw simulation. `targetRotorRPM` is derived from current throttle only while the engine can run, fuel is available, the canopy is closed, and blades are usable/unfolded. `normalizedRotorRPM` then moves toward that target using `RotorSpoolUpRate` while increasing, `RotorSpoolDownRate` while decreasing, and `RotorInertia` as resistance. This rotor state drives visual rotor rotation for opted-in helicopters only; legacy helicopters keep the original throttle-driven animation and lift behavior. Opted-in vertical lift now uses mass, rotor thrust, configured gravity, ceiling/vortex efficiency, a squared RPM lift-spool curve, `VerticalDrag`, and `MaxClimbRate`. W/S are treated as collective-only controls in the new helicopter model; they do not directly request forward/backward horizontal thrust. Forward movement comes primarily from pitch attitude feeding horizontal rotor thrust, while sideways movement comes from roll attitude. The new model calculates pitch-derived and roll-derived thrust independently so roll controls can stay immediate while lateral acceleration, drag, and speed are tuned separately from forward flight. Backward pitch can also use optional weaker thrust and speed caps without changing collective, hover mode, roll authority, cyclic authority, or pilot input responsiveness. New-model hover assist uses `HoverAssistStrength` to add bounded collective and cyclic corrections that resist altitude changes, pitch/roll attitude error, and drift without overriding active pilot input. Hover/gunner altitude hold estimates the collective needed to counter weight using the same rotor thrust, rotor RPM/spool curve, mass, gravity, vertical drag, ceiling efficiency, pitch/roll attitude, disk tilt, translational lift, and vortex-ring values as the physics step, then clamps per-tick correction so mode transitions do not snap throttle/collective. Mode switches preserve current throttle and vehicle orientation; camera/freelook pitch is not written back into vehicle pitch.
