...
| `HorizontalRotorThrustScale` / `HelicopterHorizontalThrustScale` | float >= 0 | 0.35 | New-model-only Minecraft-scale multiplier for converting rotor thrust into horizontal acceleration. Forward movement primarily comes from pitch attitude, while sideways movement comes from roll attitude. |
| `HelicopterLateralThrustScale` | float >= 0 | 0.45 | New-model multiplier applied only to roll/right-axis translational thrust. Lower values keep roll controls responsive while reducing sideways acceleration. |
| `HelicopterLateralDrag` | float >= 0 | 0.04 | New-model additional damping applied only to lateral velocity after normal parasite drag. |
| `HelicopterMaxLateralSpeedScale` | float >= 0 | 0.45 | New-model lateral speed cap as a fraction of the normal horizontal safety limit. Only the side/right velocity component is clamped. |
| `HelicopterBackwardThrustScale` | float >= 0 | 0.55 | New-model multiplier applied only when pitch/rotor thrust is driving backward flight. |
| `HelicopterMaxBackwardSpeedScale` | float >= 0 | 0.45 | New-model backward speed cap as a fraction of the normal horizontal safety limit. Only the backward component is clamped. |
...
HelicopterLateralDrag = 0.04
HelicopterMaxLateralSpeedScale = 0.45
HelicopterBackwardThrustScale = 0.55
HelicopterMaxBackwardSpeedScale = 0.45
