...
- Ship rack eligibility: `CanMountShip = false` prevents this vehicle from mounting racks on ship entities, which is useful for oversized aircraft such as strategic bombers that should not land on carriers. The default is `true` for legacy content-pack compatibility.
- External payload limits: `MaximumExternalPayloadCapacity` sets how much vehicle weight this vehicle can chain-tow or carry on racks in pounds, and `Weight` sets this vehicle's own chain/rack payload weight in pounds. Defaults are `0` lb capacity and `50000` lb weight, so vehicles cannot chain-lift or rack-carry other configured vehicles unless a payload capacity is explicitly configured. Planes cannot use chains because chain towing is treated as cargo-hook transport.
