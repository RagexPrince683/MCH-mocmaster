...
* **Free Look** (`KeyFreeLook`, default Left Control): by default this remains the original toggle action, including in regular third person, so the view does not snap back when the key is released. Enabling **Hold Free Look** in the controls menu changes it to a held action that turns off on key release. Plane left/right controls (`KeyLeft`/`KeyRight`, default A/D) still steer the aircraft while free look is active; free look only decouples the camera from mouse-driven aircraft aim.
