...
Conventional guided-bomb references use real standard gravity converted into the weapon simulation's per-tick velocity increment. Standard gravity is `9.80665 m/s²`; with Minecraft's 20 game ticks per second and the weapon code applying `Gravity` once per tick, the real-scale value is `9.80665 / 20² = 0.024516625`, rounded in configs to `Gravity = -0.02452`. This makes JDAM/LGB/KAB-style bomb drops fall like ballistic stores instead of retaining the shallow glide path that old `-0.001` to `-0.02` values created. Purpose-built glide weapons such as UMPK, JSOW, and SDB-style references keep their lower-drag/glide tuning separately.
