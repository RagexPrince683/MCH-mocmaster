...
Conventional guided-bomb and normal ballistic bomb references use real standard gravity converted into the weapon simulation's per-tick velocity increment. Standard gravity is `9.80665 m/s²`; with Minecraft's 20 game ticks per second and the weapon code applying `Gravity` once per tick, the real-scale value is `9.80665 / 20² = 0.024516625`, rounded in configs to `Gravity = -0.02452`. This makes JDAM/LGB/KAB-style and unguided bomb drops fall like ballistic stores instead of retaining the shallow glide path that old `-0.001` to `-0.02` values created. Purpose-built glide weapons such as UMPK, JSOW, and SDB-style references keep their lower-drag/glide tuning separately, while mines, tanks, dispensers, and other non-ballistic special payloads keep their specialized water or gameplay tuning.
...
**How it differs from original MCHeli:** Overdrive stores NewUAV station/location data in `data/mcheli_new_uavs.json`, tracks destroyed stations, restores item identity/damage, restores a NewUAV operator inventory if the linked station is destroyed mid-control, keeps the linked station chunk loaded while an active NewUAV is piloted, and resolves duplicate live entities through canonical selection instead of blindly deleting potentially recoverable vehicles. New UAV operators can briefly use their normal inventory near the linked station to refuel or rearm, but receive chat warnings at 10 seconds, 5 seconds, and timeout before the inventory is stored and cleared; moving more than 15 blocks from the station stores and clears it immediately.
