...
Left-click empty editor space and drag at least three logical GUI pixels to draw a rectangle around movable HUD elements. The selection updates while dragging and remains active on release. Drag normally to replace the selection, hold either Shift key to add to the selection that existed when the rectangle began, or hold either Control key to toggle touched elements relative to that starting selection. A click, or movement below the threshold, on empty space clears the selection. Fixed aiming, world-projected, and full-screen elements are excluded.
