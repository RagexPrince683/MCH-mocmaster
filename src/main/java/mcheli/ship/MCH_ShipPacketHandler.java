    ...
    public static void onPacket_PlayerControl(EntityPlayer player, ByteArrayDataInput data) {
        if(!player.worldObj.isRemote) {
            ...
            if(player.ridingEntity instanceof MCH_EntityShip) {
                plane = (MCH_EntityShip)player.ridingEntity;
            } else if(player.ridingEntity instanceof MCH_EntitySeat) {
                if(((MCH_EntitySeat)player.ridingEntity).getParent() instanceof MCH_EntityShip) {
                    plane = (MCH_EntityShip)((MCH_EntitySeat)player.ridingEntity).getParent();
                }
                ...
                if(ac.getControlAircract() instanceof MCH_EntityShip) {
                    plane = (MCH_EntityShip)ac.getControlAircract();
                }
            }
            ...
            mcheli.aircraft.MCH_BaseVehiclePacketHandler.handleRadarToggle(player, plane, pc);
                if(pc.isUnmount == 1) {
                    if(mcheli.aircraft.MCH_BaseVehiclePacketHandler.validateNormalDismount(
                          player, plane, pc.dismountMountEntityId, pc.dismountParentEntityId, pc.dismountSeatId)) {
                       plane.unmountEntity();
                    }
