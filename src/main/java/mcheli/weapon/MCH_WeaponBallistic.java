...
public class MCH_WeaponBallistic extends MCH_WeaponBase{
	public MCH_WeaponBallistic(World w, Vec3 v, float yaw, float pitch, String nm, MCH_WeaponInfo wi) {
	...
	public boolean shot(MCH_WeaponParam prm) {
		...
		MCH_EntityBallisticMissile e;
		if(prm.entity instanceof MCH_EntityAircraft) {
			if(((MCH_EntityAircraft)prm.entity).target != null) {
				MCH_EntityAircraft plane = ((MCH_EntityAircraft)prm.entity);
				if(plane.getDistance(plane.target.xCoord, plane.posY, plane.target.zCoord) > this.getInfo().radius) {return false;}
				e = new MCH_EntityBallisticMissile(this.worldObj, prm.posX, prm.posY, prm.posZ, tX, tY, tZ, yaw, pitch, this.acceleration);
				e.setName(this.name);    
				e.setParameterFromWeapon(this, prm.entity, prm.user);
				e.setTarget(plane.target.xCoord, plane.target.yCoord, plane.target.zCoord);
