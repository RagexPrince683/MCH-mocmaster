...
public class MCH_ClientCommonTickHandler extends W_TickHandler {
   ...
   private static void debugFlightControl(MCH_EntityBaseVehicle ac, float simDelta, float mouseX, float mouseY, float stickX, float stickY) {
      ...
      if(heli != null && heli.isNewHeliFlightModelEnabled()) {
         ...
         System.out.println(String.format("[MCHeli][NewHeli][cyclic/horizontal] input=(pitch=%.4f,roll=%.4f) tilt=(forward=%.4f,right=%.4f) thrust=(x=%.4f,z=%.4f) accel=(x=%.4f,z=%.4f) dragOrCapDelta=(x=%.4f,z=%.4f) velocityComponents=(forward=%.4f,lateral=%.4f) lateralThrustScale=%.4f lateralCap=%s backwardScaleOrCap=%s",
               Float.valueOf(heli.getCyclicPitchInput()), Float.valueOf(heli.getCyclicRollInput()), Float.valueOf(heli.getRotorTiltForward()), Float.valueOf(heli.getRotorTiltRight()), Float.valueOf(heli.getRotorHorizontalThrustX()), Float.valueOf(heli.getRotorHorizontalThrustZ()), Float.valueOf(heli.getHorizontalAccelerationX()), Float.valueOf(heli.getHorizontalAccelerationZ()), Float.valueOf(heli.getParasiteDragAppliedX()), Float.valueOf(heli.getParasiteDragAppliedZ()), Float.valueOf(heli.getForwardVelocityComponent()), Float.valueOf(heli.getLateralVelocityComponent()), Float.valueOf(heli.getAppliedLateralThrustScale()), Boolean.valueOf(heli.isLateralSpeedCapActive()), Boolean.valueOf(heli.isBackwardFlightScalingActive())));
