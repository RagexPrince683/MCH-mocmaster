...
public class MCH_WheelManager {
   ...
   public void move(double x, double y, double z) {
      ...
      if(ac.getAcInfo() != null) {
         ...
         for(wc = 0; wc < rv; ++wc) {
            pitch = zmog[wc];
            // KEEP ORIGINAL MOVEMENT BEHAVIOR (single move + small downward nudge)
            pitch.motionY *= 0.15D;
            pitch.moveEntity(pitch.motionX, pitch.motionY, pitch.motionZ);
            pitch.moveEntity(0.0D, -0.1D, 0.0D);
         }
         ...
         //// ------------------- SAFETY: rate-limited rotation + iterative ground-check -------------------
         //// original rotation interpolation factor (kept)
         //double dx = super.posX - super.prevPosX; // not used here but matches original style
         //double dz = super.posZ - super.prevPosZ;
         //double dist = dx * dx + dz * dz; // not used directly
         //so its not used in here so why is it here, just schizo babbel

         // interpolation alpha (kept similar to previous attempts)
         float sf = 1.0F;
         float alphaRot = 0.22F;

         // speed-dependent max delta (conservative)
         ...
         float maxDeltaLow = 3.5F;
         float maxDeltaHigh = 1.0F;
         ...
         // Iteratively reduce the deltas if applying them would put any wheel below the ground
         float applyPitch = pitchDelta;
         float applyRoll = rollDelta;
         int maxIter = 6;
         for (int iter = 0; iter < maxIter; iter++) {
            boolean collision = false;
            float testPitch = ac.getRotPitch() + applyPitch;
            float testRoll  = ac.getRotRoll()  + applyRoll;
            // check all wheels projected under that rotation
            for (int wi = 0; wi < this.wheels.length; wi++) {
               MCH_EntityWheel w = this.wheels[wi];
               Vec3 v = this.getTransformedPosition(w.pos.xCoord, w.pos.yCoord, w.pos.zCoord, ac, ac.getRotYaw(), testPitch, testRoll);
               int bx = MathHelper.floor_double(v.xCoord + 0.5D);
               int bz = MathHelper.floor_double(v.zCoord + 0.5D);
               int startY = MathHelper.floor_double(v.yCoord + 1.0D);
               int groundY = Integer.MIN_VALUE;
               // small depth search for top solid block under wheel point
               int searchDepth = 4;
               for (int yy = startY; yy >= startY - searchDepth; yy--) {
                  Block b = ac.worldObj.getBlock(bx, yy, bz);
               ...
               if (groundY != Integer.MIN_VALUE) {
                  double clearance = 0.22D; // keep a small clearance
                  double minAllowed = groundY + 0.5D + clearance;
                  if (v.yCoord < minAllowed) {
                     collision = true;
                     break;
                  }
               }
            ...
            if (!collision) break;
            // reduce deltas and retry
            applyPitch *= 0.5F;
            applyRoll  *= 0.5F;
         }

         if(!W_Lib.isClientPlayer(ac.getRiddenByEntity())) {
            ac.setRotPitch(ac.getRotPitch() + applyPitch);
            ac.setRotRoll(ac.getRotRoll() + applyRoll);
         }
