...
public class MCH_EntityWheel extends W_Entity {
   ...
   public void moveEntity(double parX, double parY, double parZ) {
      ...
      // Clamp very large vertical requests to avoid teleportation that leads to collision feedback.
      // This prevents wheels from being moved hundreds of blocks in a single tick by collision correction.
      double maxVerticalMovePerTick = 1.5D; // tune down if needed
      if (parY > maxVerticalMovePerTick) parY = maxVerticalMovePerTick;
      if (parY < -maxVerticalMovePerTick) parY = -maxVerticalMovePerTick;
